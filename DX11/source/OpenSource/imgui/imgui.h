...
//! Using dear imgui via a shared library is not recomend_ed, because of function call overhead and because we don't guarantee backward nor forward ABI compatibility.
#ifndef IMGUI_API
#define IMGUI_API
#endif
...
namespace ImGui
{
    ...
    //! Styles
    IMGUI_API void          StyleColorsDark(ImGuiStyle* dst = NULL);    //! new, recomend_ed style (default)
    ...
    IMGUI_API void          SetWindowPos(const ImVec2& pos, ImGuiCond cond = 0);                        //! (not recomend_ed) set current window position - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-effects.
    IMGUI_API void          SetWindowSize(const ImVec2& size, ImGuiCond cond = 0);                      //! (not recomend_ed) set current window size - call within Begin()/End(). set to ImVec2(0,0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-effects.
    IMGUI_API void          SetWindowCollapsed(bool collapsed, ImGuiCond cond = 0);                     //! (not recomend_ed) set current window collapsed state. prefer using SetNextWindowCollapsed().
    IMGUI_API void          SetWindowFocus();                                                           //! (not recomend_ed) set current window to be focused / top-most. prefer using SetNextWindowFocus().
    ...
    //! Widgets: Text
    IMGUI_API void          TextUnformatted(const char* text, const char* text_end = NULL); //! raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recomend_ed for long chunks of text.
...
enum ImGuiConfigFlags_
{
    ...
    //! [BETA] Viewports
    //! When using viewports it is recomend_ed that your default value for ImGuiCol_WindowBg is opaque (Alpha=1.0) so transition to a viewport won't be noticeable.
